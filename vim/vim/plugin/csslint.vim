...
if !exists("css_command")
    let css_command = 'csslint'
endif

if !exists("css_command_options")
    let css_command_options = '--quiet'
endif

if !exists("css_highlight_color")
    let css_highlight_color = 'DarkMagenta'
endif

" set up auto commands
autocmd BufWritePost,FileWritePost *.css call CSS_Lint()
...
function CSS_Lint() 
    ...
    let cmd_output = system(g:css_command . ' ' . g:css_command_options . ' ' . current_file)
...
function s:SetCursorLineColor() 
    ...
    if(!exists("g:css_highlight_color") || strlen(g:css_highlight_color) == 0) 
        return 
    endif
    ...
    execute "highlight CursorLine guibg=" . g:css_highlight_color
endfunction
